...
from more_information import more_information
...
def main():
    ...
    for username in all_usernames:
        ...
        if args.output:
            ...
            result_path = args.folderoutput
        else:
            result_file = f"{username}.txt"
            result_path = '.\\'

        m_info = more_information()
        with open(result_file, "w", encoding="utf-8") as file:
            exists_counter = 0
            for website_name in results:
                dictionary = results[website_name]
                if dictionary.get("status").status == QueryStatus.CLAIMED:
                    if website_name in m_info.site_list:
                        m_info.get_information(site=website_name, username=username, path=result_path)
                    exists_counter += 1
